...
internal sealed class ObligationReminderReceiver : BroadcastReceiver
{
    public const string ActionReminder = "controleo.mobile.action.OBLIGATION_REMINDER";
    public const string ChannelId = "obligation-reminders";
    public const string ChannelName = "Recordatorios de obligaciones";
    ...
    public override void OnReceive(Context? context, Intent? intent)
    {
        ...
        if (!string.Equals(intent.Action, ActionReminder, StringComparison.Ordinal))
        {
            return;
        }

        var notificationId = intent.GetIntExtra(ExtraNotificationId, 1001);
        var title = intent.GetStringExtra(ExtraTitle) ?? "Recordatorio de pago";
        var body = intent.GetStringExtra(ExtraBody) ?? "Tienes una obligación próxima a vencer.";
    ...
    private static void EnsureNotificationChannel(NotificationManager manager)
    {
        ...
        {
            Description = "Recordatorios de vencimiento de obligaciones premium."
        };
