...
public interface IExpenseStorageService
{
    ...
    Task<PagedExpenseResult> GetExpensesPageAsync(string monthKey, int pageNumber, int pageSize, string? movementType, CancellationToken cancellationToken);
    Task<IReadOnlyList<string>> GetAvailableMonthKeysAsync(CancellationToken cancellationToken);
